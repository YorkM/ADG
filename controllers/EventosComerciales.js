...
const listarEventosCierres = async (descripcion) => {
  try {
    const respuesta = await enviarPeticion({link: `${url_api}eventoscierre/search`, descripcion});
    let p = `<option value="0">No</option>`;

    if (respuesta.length) respuesta.forEach(item => p += `<option value="${item.id}">${item.descripcion.toUpperCase()}</option>`);
...
const ApilistarOficinas = async () => {
  try {
    ...
    if (data.length) {
      const oficinasFiltradas = data.filter(item => item.oficina_ventas.substring(0, 1) === oficina.substring(0, 1));
      oficinasFiltradas.forEach(item => p += `<option value="${item.oficina_ventas}">${item.oficina_ventas} - ${item.descripcion}</option>`);
...
const crearEventoCierre = async () => {
  try {
    ...
    const respuesta = await enviarPeticion({link: url_api + 'eventoscierre/create', descripcion});
...
const mostrarBanners = (imagenes) => {
  imagenes.forEach((img, idx) => $(`#${img}`).attr("src", `${url_img}banner1_evento_comercial_${(idx + 1)}`));
}
// FUNCIÓN PARA CARGAR FORMULARIO
const cargarForm = async (id) => {
  try {
    showLoadingSwalAlert2('Cargando información', false);
    const respuesta = await enviarPeticion({link: `${url_api}eventoscomerciales/searchid`, metodo: 'GET', id});
...
const eliminarLabSave = async (id, ob) => {
  try {
    const result = await confirmAlert("¿Está seguro(a) que desea eliminar el laboratorio?");
    if (result.isConfirmed) {
      if (id > 0) {
        const resp = await enviarPeticion({link: `${url_api}laboratoriosconvocatoria/eliminarporid`, id});
        if (resp.ok) {
          Swal.fire("Eliminar laboratorio", "Se eliminó correctamente el laboratorio", "success");
...
const HttpEliminarLabsSave = async (id_evento) => {
  return await enviarPeticion({link: `${url_api}laboratoriosconvocatoria/eliminarporevento`, id_evento});
}
...
const EliminarLabsSave = async () => {
  try {
    const result = await confirmAlert("Eliminar laboratorio", "¿Está seguro(a) que desea eliminar todos los laboratorios?");
    if (result.isConfirmed) {
      const id_evento = $("#id-evento-modal").val();
      if (id_evento > 0) {
        const resp = await HttpEliminarLabsSave(id_evento);
        if (resp.ok) {
          Swal.fire("Eliminar laboratorios", "Se eliminaron correctamente los laboratorios", "success");
          $("#list-lab-modal").html('');
        } else {
...
const abrirModalLab = async (id_evento) => {
  try {
    ...
    const resp = await enviarPeticion({link: `${url_api}laboratoriosconvocatoria/buscarporevento`, id_evento});
...
const verConsolidadoOficinas = (resp) => {
  ...
  objOficinas.forEach(item => {
    ...
    <tr>
      ...
      <td class="size-15" style="background-color: ${bgColor};">${item.porcentajeCumplimiento}</td>
    </tr>`;
  });
...
const buscarEventos = async () => {
  try {
    ...
    respuesta.forEach((item, index) => {
      ...
      if (item.estado == 'I') estado = '<span class="bg-danger badge ">Inactivo</span>';
      ...
      <tr>
        ...
        <td class="text-center">
          <button class="btn btn-sm btn-outline-success btn-presupuesto" data-id='${JSON.stringify(item)}'>
            <i class="fa-solid fa-money-bill-1" style="font-size: 15px;"></i>
          </button>
        </td>                       
        <td class="text-center">
          <button class="btn btn-sm btn-outline-secondary btn-portafolio" data-id='${JSON.stringify(item)}'>
            <i class="fa-solid fa-briefcase" style="font-size: 15px;"></i>
          </button>
        </td>                       
        <td class="text-center">
          <button class="btn btn-sm btn-outline-primary btn-seguimiento" data-id="${item.id}">
            <i class="fa-solid fa-record-vinyl" style="font-size: 15px;"></i>
          </button>
        </td>
        <td class="text-center">
          <button class="btn btn-sm btn-outline-warning" onclick="cargarForm(${item.id})">
            <i class="fa-regular fa-pen-to-square" style="font-size: 15px;"></i>
          </button>
        </td>             
        <td class="text-center">
          <button class="btn btn-sm btn-outline-danger" onclick="eliminarEvento(${item.id},this)">
...
const updateImgBanner = async (id, nombre, campo) => {
  const respuesta = await enviarPeticion({link: `${url_api}eventoscomerciales/updateimgbanner`, nombre, campo, id});
}
...
const eliminarEvento = async (id, ob) => {
  try {
    const result = await confirmAlert("Eliminar evento cierre", "¿Está seguro(a) que desea eliminar el evento?");
    if (result.isConfirmed) {
      const respuesta = await enviarPeticion({id, link: `${url_api}eventoscomerciales/delete/`, metodo: 'GET'});
      if (respuesta.ok) {
...
const limpiarListLab = async (op) => {
  const result = await confirmAlert("Eliminar laboratorios", "¿Está seguro(a) que desea eliminar los laboratorios agregados?");
  if (result.isConfirmed) {
    if (op == 1) {
      $("#list-lab").html('');
      $("#lab-csv").val('');
