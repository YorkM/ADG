...
const getReporteVisitasZonas = async () => {
	...
	let oficina = (oficinaSubs === "20") ? "2" : oficinaSubs;
		...
		success: function (data) {
			...
			if (data != '' && data != null) {
				...
				let data_motivo_visita = data.forEach(item => {
					...
					if (item.RAZON_IMPREVISTO == 'V') { cont_razon_venta++; } else
						if (item.RAZON_IMPREVISTO == 'C') { cont_razon_cartera++; } else
							if (item.RAZON_IMPREVISTO == 'P') { cont_razon_pqrs++; } else
								if (item.RAZON_IMPREVISTO == 'S') { cont_razon_pos++; } else
									if (item.RAZON_IMPREVISTO == 'O') { cont_razon_otro++; } else {
										cont_razon_otro++;
									}
