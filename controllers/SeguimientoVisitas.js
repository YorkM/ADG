...
$(function() {
	$("#eliminar_programacion").click(async function(){
		try {
			...
			if(confirm("Está seguro de que desea eliminar las programaciones? ")){
			   ...
			   if(ids.length==0){
				   swal("Info","No se encontraron datos seleccionados","waring");
			   }
	...
	TipoUsuario();	
	CargarEjecutivos();
	$("#tipo").change(function(){CargarEjecutivos();});	
	///oficina de ventas
		   ...
		   //alert(id_ven+'   '+cod_sap_cli);
		    $.ajax({
					...
					success: function(data){
						//alert(data);
						if(data>0){
							 swal("Ok", "Se programó correctamente el cliente "+cliente+" para POSVENTA !", "success"); 
						}
		            }
 ...
 */
 ...
 $.ajax({
		...
		success: function(data){ 
			...
			if(data.length==0){  
				$("#Result_prog_v").html(`<div class="alert alert-warning"><i class="fa fa-search m-1"></i>No se encontraron programaciones gestionadas no contactadas</div>`); return; 
			}
				...
				if($.trim(data[i].COMENTARIO_VENTA)!=""){
					rv = "<img src='../resources/icons/ProgVentas1.png' align='absmiddle' width='16' height='16'/>";
					   ...
					   switch(data[i].REVISION_VENTAS){
						  case '0':{rv = "<img src='../resources/icons/CirculoRojo.png' align='absmiddle' width='12' height='12'/>";}break;
						  case '1':{rv = "<img src='../resources/icons/CirculoVerde.png' align='absmiddle' width='12' height='12'/>";}break;
					   }					
				}
				//
                if($.trim(data[i].COMENTARIO_CARTERA)!=""){
					rv = "<img src='../resources/icons/ProgCartera1.png' align='absmiddle' width='16' height='16'/>";
					   ...
					   switch(data[i].REVISION_CARTERA){
						  case '0':{rc = "<img src='../resources/icons/CirculoRojo.png' align='absmiddle' width='12' height='12'/>";}break;
						  case '1':{rc = "<img src='../resources/icons/CirculoVerde.png' align='absmiddle' width='12' height='12'/>";}break;
					   }					
				}
				//
				if($.trim(data[i].COMENTARIO_JURIDICA)!=""){
					rv = "<img src='../resources/icons/ProgJuridica1.png' align='absmiddle' width='16' height='16'/>";
						...
						switch(data[i].REVISION_JURIDICA){
						  case '0':{rj = "<img src='../resources/icons/CirculoRojo.png' align='absmiddle' width='12' height='12'/>";}break;
						  case '1':{rj = "<img src='../resources/icons/CirculoVerde.png' align='absmiddle' width='12' height='12'/>";}break;
					   }
				}

			   switch(data[i].ESTADO_VISITA){
					   
				  case 'P':{es = '<button type="button" class="btn btn-danger btn-xs" aria-label="Left Align">'+
								   '<span class="fa fa-times-circle" aria-hidden="true"></span>'+
								 '</button>'; 
							CP++;
				  }break;
				  case 'V':{es = '<button type="button" class="btn btn-info btn-xs" aria-label="Left Align">'+
				  ...
				  case 'C':{es = '<button type="button" class="btn btn-success btn-xs" aria-label="Left Align">'+
								   '<span class="fa fa-check-circle aria-hidden="true"></span>'+
								 '</button>'; 
						    if(data[i].TIPO_PROGRAMACION!='S'){
								  CantVis++; 
					     ...
					     '<td colspan="17" align="center" class="form disabled">'+
						   ...
						   if(data[i].TIPO_PROGRAMACION!='S'){
								...
								tabla+='<tr class="">'+
									   ...
									   '</tr>'+
									   '<tr class="">'+
										  '<td width="10%"><b>GESTION LLAMADA</b></td>'+/*0*/
										  '<th width="10%" colspan="3" align="left">'+
							     ...
							     tabla+='<tr >'+
								  ...
								  /*2*/
								  tabla+='<th style="display:none">'+data[i].EXT+'</th>'+//3
										 '<th style="display:none">'+data[i].ID_VEN+'</th>'+/*4*/
										 '<th style="display:none">'+data[i].FECHA_VISITA+'</th>'+//5
										 '<th style="display:none">'+data[i].CODIGO_SAP_CLI+'</th>'+//6RptVisitasLlamadas
    ...
    $.ajax({
		...
		for(var i=0; i<=data.length-1; i++){
			...
			$("#Result3").tableExport({
		     ...
		     swal('Oops','No hay datos que exportar.','warning');
		  }
...
/*
  getAddressFromCoordinates(lat0, lng0, 'AIzaSyAiEVx_nyyrkNci_hWB_fMuUAkKn_OH_b8')
  .then(res=>{
	console.log({
		resp
	})
...
function ShowGestion(ob,id,rv,rj,rc){
	 ...
	 switch($("#RolId").val()){
		case '3':{//Ventas
			if(rv == '0'){ActualizarEstado('V',id);}
		}break;
		case '1':{//Ventas
			console.log('CASO 3')
			if(rv == '0'){ActualizarEstado('V',id);}
		}break;
		case '13':{//Coordinadores contact center
			if(rv == '0'){ActualizarEstado('V',id);}
		}break;
		case '12':{//Coordinadores contact center
			if(rv == '0'){ActualizarEstado('V',id);}
		}break;
	   case '17':{//Cartera
		   if(rc == '0'){ActualizarEstado('C',id);}
	   }break;
	   case '18':{//Juridica
		   if(rj == '0'){ActualizarEstado('J',id);}
	   }break;
	 }
}
...
function GuardarGestion(id,ob){
	...
	if($(ob).val()!='' && dialog!=''){
		 $.ajax({
			...
			success: function(data){
			  if(data=='error'){
				  swal('Error!','Error al intentar guardar el comentario!','error');
			  }else{
...
function AutorizarCoordenadas(){
  ...
  $.ajax({
		...
		success: function(data){
		   if(data == 1){
			 swal('Excelente','Solicitud autorizada con exito.','success'); 
		   }else{
			 swal('Error','Su solicitud no pudo ser autorizada, verifique e intente nuevamente','error'); 
		   }
		   Consultar()
		}	
 });
