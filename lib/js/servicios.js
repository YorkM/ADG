...
//url solicitudes de ingresos
 function enviarPeticion(data){	
	...
	return new Promise((resolve,reject)=>{
	  ...
	  metodo='POST'
      if(data.link == undefined ){ reject('No se envio el link');  }
	  if(data.metodo!=undefined){ metodo=data.metodo }
		
	  $.ajax({     
		    url		: 	data.link,/* url o donde esta el php que va a ejecutar la consulta sql */
			type	: 	metodo,/* tipo de dato enviado , como se envia */
			data 	: 	(data),
			encoding: "UTF-8",
			dataType: 	"json",/*tipo de dato de respuesta*/
			async	:	true,
...
async function getListMonth(){	
	...
	return new Promise((resolve,reject)=>{
	  ...
	  $.ajax({     
		    url		: 	"../models/Servicios.php",/* url o donde esta el php que va a ejecutar la consulta sql */
			type	: 	"POST",/* tipo de dato enviado , como se envia */
			data 	: 	({  op:"get_month"  }),
			dataType: 	"json",/*tipo de dato de respuesta*/
			async	:	true,
			success	: function(data)/*respuesta */
...
function srvDeleteDetalle(idfila,codigo,numero,oficina,lista,canal){
	...
	return new Promise((resolve,reject)=>{
		 ...
		 $.ajax({
			  ...
			  reject(data[0]);console.error(data);
		 });
	});
				...
				success : function(data){
					if(data.length>0){
						...
						if(!data.error){
						 ...
						 resolve([{error:false,mensaje:'no hay datos que mostrar ',data:[]}]);
					 }
					

				}
		 }).fail(function(data){ console.error(data)
			 reject([{error:true,mensaje:data,data:[]}]); console.error("error del serv",data); console.error(data);
		 });
...
async function viewListEvaluacionesPendientes(top,id){
	...
	.then(resp=>{ 		
		...
		li+=`<li class="list-group-item text-center btn "
					data-bs-toggle="modal" 
					data-bs-target=".modal_list_evaluaciones_pendientes" 
				    onclick="viewInTableEvaluacionesPendientes(10,${id})"><ins>Ver todas</ins></li>`;
...
function buscarDetalle(id,link){
	...
	.then(resp=>{
		...
		if(resp.length==0){ $("#result_detalle").html(`<div class="alert alert-danger">Sin resultados</div>`); return; }
...
function visualizarSolicitudesIngresos(resp){
  ...
  $(".n_solicitudes_ing").html(resp.length)	
   ...
   if(resp.length == 0){  	
	   ...
	   if( !$("#ModalPendientes").is(":visible") ){$("#ModalPendientes").modal("show");}
	} 
...
const validateFileUpload =(archivo,options)=>{
	...
	if(options.ruta==''){ throw new Error('El parametro ruta es necesario'); }
...
const showLoadingSwalAlert2=(html = '', WhithImg = true, sw = false) => {
		 // Bloquear la pantalla
	img ='<img src="../resources/icons/preloader.gif"  class="img-fluid"/>';
	if(!WhithImg){
	   img=html
	 }
	Swal.fire({
	  title				: img,
	  //html				: '<i class="spinner-border spinner"></i>',
...
const datosPerfil =async (id)=>{
	try{
		...
		switch(datos.oficina_ventas){
			case '1100': ofi='Monteria'; break;
			case '1200': ofi='Cartagena'; break;
			case '2100': ofi='Medellín'; break;
			case '2200': ofi='Bogotá'; break;
			case '2300': ofi='Cali'; break;
			case '2400': ofi='Barranquilla'; break;
 ...
 */
  const createDataTable = (
	...
	const { callback, clases, customClass, icon, rowAmount, order, dom, buttons } = config;
	...
	if (config) {
		defaultButton+=`<button class="${clases} ${customClass}">
			${icon}
		</button>`;
	}
	
	if(typeof editCallback == 'function' || typeof deleteCallback == 'function' || typeof callback == 'function'){
	...
	$(`#${tableId} tbody`).on('click', `button.${customClass}`, function () {
	  const data = $(`#${tableId}`).DataTable().row($(this).parents('tr')).data();
	  callback(data);
	});
