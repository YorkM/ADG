...
switch ($_POST['op']) {
	case 'CargarClienteLogin':
		$sql = "SELECT T.NOMBRES+'-'+T.RAZON_COMERCIAL AS value, T.NIT, T.NIT_DIGITO, T.NIT_TIPO, 
		T.PERFIL_TRIBUTARIO, T.CODIGO_SAP, T.NOMBRES, ISNULL(T.SEXO,'N') AS SEXO, T.FECHA_CREACION,
		T.FECHA_NACIMIENTO,	T.FECHA_MODIFICACION, T.TELEFONO1, T.TELEFONO2, T.TELEFONO3, T.EMAIL,
		ISNULL(t.RAZON_COMERCIAL,'') AS RAZON, T.CUPO_CREDITO, O.CONDICION_PAGO, O.LISTA,
		V.NOMBRES AS VENDEDOR, L.NOMBRES AS TELEVENDEDOR, ISNULL(T.EXCENTO_IVA,0) AS EXCENTO_IVA,
		ISNULL(T.EXCENTO_CREE,0) AS EXCENTO_CREE, ISNULL(T.RESPONSABLE_ICA,0) AS RESPONSABLE_ICA,
		ISNULL(O.GRAN_CONTRIBUYENTE,0) AS GRAN_CONTRIBUYENTE, O.AUTORETENEDOR, O.ZONA_VENTAS,
		O.CANAL_DISTRIBUCION, O.OFICINA_VENTAS, O.SECTOR, ISNULL(O.INSTITUCIONAL,0) AS INSTITUCIONAL,
		ISNULL(O.CONTROLADOS,0) AS CONTROLADOS,  T.DIRECCION, T.CIUDAD, T.DEPARTAMENTO, T.LONGITUD, T.LATITUD
		FROM T_TERCEROS t 
		...
		WHERE O.ORGANIZACION_VENTAS = " . mb_strtoupper($_POST["org"], "UTF-8") . " AND 
	...
	case 'B_CLIENTES':
		$sql = "SELECT TOP 5 T.NOMBRES+'-'+T.RAZON_COMERCIAL AS value, T.NIT, T.NIT_DIGITO, T.NIT_TIPO,
		T.PERFIL_TRIBUTARIO, T.CODIGO_SAP, T.NOMBRES, ISNULL(T.SEXO,'N') AS SEXO, T.FECHA_CREACION,
		T.FECHA_NACIMIENTO, T.FECHA_MODIFICACION, T.TELEFONO1, T.TELEFONO2, T.TELEFONO3, ISNULL(T.EMAIL,'') AS EMAIL,
		ISNULL(T.EMAIL_FE,'') AS EMAIL_FE, ISNULL(t.RAZON_COMERCIAL,'') AS RAZON, T.CUPO_CREDITO, O.CONDICION_PAGO,
		O.LISTA, V.NOMBRES AS VENDEDOR, L.NOMBRES AS TELEVENDEDOR, ISNULL(T.EXCENTO_IVA,0) AS EXCENTO_IVA,
		ISNULL(T.EXCENTO_CREE,0) AS EXCENTO_CREE, ISNULL(T.RESPONSABLE_ICA,0) AS RESPONSABLE_ICA,
		ISNULL(O.GRAN_CONTRIBUYENTE,0) AS GRAN_CONTRIBUYENTE, O.AUTORETENEDOR, O.ZONA_VENTAS, O.CANAL_DISTRIBUCION,
		O.OFICINA_VENTAS, O.SECTOR, ISNULL(O.INSTITUCIONAL,0) AS INSTITUCIONAL, ISNULL(O.CONTROLADOS,0) AS CONTROLADOS,
		T.DIRECCION, T.CIUDAD, T.DEPARTAMENTO, T.LONGITUD, T.LATITUD, T.NOTIFICACION_CARTERA, T.NOTIFICACION_FECHA
		FROM T_TERCEROS t 
		INNER JOIN T_TERCEROS_ORGANIZACION O ON O.CODIGO_SAP = T.CODIGO_SAP
		LEFT  JOIN T_TERCEROS L ON L.CODIGO_SAP = O.CODIGO_TELEVENDEDOR
		LEFT  JOIN T_TERCEROS V ON V.CODIGO_SAP = O.CODIGO_VENDEDOR
	...
	case 'B_DIRECCIONES':
		$sql = "SELECT D.ID, D.DIRECCION, P.DESCRIPCION AS DPTO, C.DESCRIPCION AS CIUDAD
		...
		WHERE D.CODIGO_SAP = '" . $_POST['codigo'] . "'";
	...
	case 'B_ACUERDOS': 
		$sql = "SELECT T.ID, vlr = (select sum(vs.p_neto) from v_facturacion vs 
		WHERE (vs.tipo_factura LIKE 'ZF%' OR vs.tipo_factura LIKE 'ZN%') 
		AND vs.fecha BETWEEN t.FECHA_INI AND t.FECHA_FIN 
		AND vs.organizacion_ventas = '" . $_POST['org'] . "' 
		AND vs.CODIGO_SAP = t.CODIGO_SAP ), T.ADJUNTO, T.ID, T.NUMERO, T.FECHA_INI, T.FECHA_FIN, T.INCENTIVO, T.MONTO, T.OBSERVACION, TT.NOMBRES
	...
	case 'B_PAGARES':
		$sql = "SELECT T.ID, T.CODIGO_SAP, T.ADJUNTO, TT.NOMBRES 
		FROM T_PAGARES T 
		INNER JOIN T_TERCEROS TT ON(TT.CODIGO_SAP=T.CODIGO_SAP)
		WHERE T.CODIGO_SAP= '" . $_POST['codigo'] . "'";
		echo json_encode(GenerarArray($sql, ""));
	...
	case 'G_DIRECCIONES':
		$sql = "INSERT INTO T_TERCEROS_DIR (CODIGO_SAP, CIUDAD, DEPARTAMENTO, DIRECCION)
		VALUES('" . $_POST['codigo'] . "','" . $_POST['mpio'] . "','" . $_POST['dpto'] . "','" . strtoupper($_POST['dir']) . "')";

		if (mssql_query($sql)) echo 1;
		else echo 0;
		break;
	...
	case 'B_MATERIALES':
		...
		$sql = "SELECT DISTINCT TOP 5 UPPER(M.CODIGO_MATERIAL + ' - ' + ISNULL(M.DESCRIPCION2, M.DESCRIPCION)) AS value, 
		M.CODIGO_MATERIAL, ISNULL(M.DESCRIPCION2, M.DESCRIPCION) AS DESCRIPCION 
		FROM T_MATERIALES M 
	...
	case 'S_FACTURAS':
		$sql = "SELECT P.NUMERO, CAST(P.FECHA_PEDIDO AS DATE) AS FECHA_PEDIDO, ISNULL(P.NOTA,'N/A') AS NOTA,
		F.TIPO_FACTURA, F.NUMERO_FACTURA, F.FECHA_INICIO
		...
		WHERE D.CODIGO_MATERIAL = '" . $_POST['material'] . "' 
		AND P.CODIGO_SAP = '" . $_POST['codigo'] . "'
		GROUP BY P.NUMERO, P.FECHA_PEDIDO, P.NOTA, F.TIPO_FACTURA, F.NUMERO_FACTURA, F.FECHA_INICIO";
	...
	case "Load_Aficiones":
		$sql = "SELECT ID, AFICION FROM T_CRM_AFICIONES";
		echo json_encode(GenerarArray($sql, ""));
		mssql_close();
		break;
	...
	case 'G_ACUERDOS':
		...
		$sql = "INSERT INTO T_ACUERDO_COMERCIAL (CODIGO_SAP, NUMERO, FECHA_INI, FECHA_FIN, MONTO, INCENTIVO, OBSERVACION, ADJUNTO, ORGANIZACION_VENTAS, OFICINA_VENTAS)
		VALUES('" . $_POST['c_sap'] . "','" . $_POST['acu'] . "','" . $_POST['f_ini'] . "','" . $_POST['f_fin'] . "','" . $_POST['mont'] . "','" . $_POST['inc'] . "','" . strtoupper($_POST['obs']) . "','" . $nombre . "','" . $_POST['organizacion'] . "','" . $_POST['oficina'] . "')";
	...
	case "subir":
		...
		if ($tipo_archivo[1] == "pdf") {
			...
			if (!move_uploaded_file($tmp_archivo, $archivador)) echo "No se ha podido cargar el pdf .";
			else echo  $nombre;
		} else {
	...
	case "Gestiones": 
		$sql = "SELECT U.NOMBRES+' '+U.APELLIDOS AS NOMBRES, CAST(V.FECHA_VISITA AS DATE) AS FECHA_VISITA, V.OBJETIVO_CARTERA,
		V.OBJETIVO_JURIDICA, V.OBJETIVO_VENTA, V.LOGRO_CARTERA, V.LOGRO_JURIDICA, V.LOGRO_VENTA 
	...
	case "CarteraXEdades": 
		$sql = "SELECT V.CLIENTE, SUM(V.C_SIN_VENCER) AS C_SIN_VENCER, SUM(V.C_31_60) AS C_31_60,
		SUM(V.C_61_90) AS C_61_90, SUM(V.C_91_120) AS C_91_120, SUM(V.C_120) AS C_120
		FROM V_CARTERA V 
		WHERE V.CLIENTE  = '" . $_POST['codigo'] . "' 
		AND V.SOCIEDAD = '" . $_SESSION['ses_NumOrg'] . "' 
		AND V.DOCUMENTO_COMPENSACION = '-'
		GROUP BY V.CLIENTE";
	...
	case 'S_SINC_TERCEROS': 
		$sql = "SELECT top 1 LTRIM(T.KUNNR,0) AS CODIGO_SAP, C.KVGR1 AS GRUPO1, C.KVGR2 AS GRUPO2, 
		C.KVGR3 AS GRUPO3, C.KVGR4 AS GRUPO4, C.KVGR5 AS GRUPO5, T.REGIO AS COD_DPTO, 
		SUBSTRING(P.CITY_CODE, 10, 12) COD_CIUDAD, P.CITY_NAME AS NOMBRE_CIUDAD    
		FROM KNA1 T 
		INNER JOIN KNVV C ON C.KUNNR = T.KUNNR
		INNER JOIN ADRCITY D ON D.REGION = T.REGIO
		LEFT JOIN ADRCITYT P ON P.CITY_CODE = D.CITY_CODE 
		AND UPPER(REPLACE(P.CITY_NAME,'É','E')) = UPPER(T.MCOD3)
		WHERE LTRIM(T.KUNNR,0) = '" . $_POST['codigo'] . "'
		ORDER BY P.CITY_NAME DESC";
	...
	case "U_ARCHIVOS":
		...
		if ($sw === "1") {
			...
			$sql = "UPDATE T_ACUERDOS_PAGO SET $campoActualizar = '$nombreArchivo' WHERE ID = '$idProceso'";
		}
	...
	case "G_ABONOS":
		...
		CASE WHEN AP.TIPO_PAGO = '1' THEN 'ABONO' ELSE 'TOTAL' END AS TIPO, AP.SOPORTE_PAGO,
