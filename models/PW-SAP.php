...
switch ( $_POST[ 'op' ] ) {
        ...
        */  
        ...
        if ( $clase == 'ZPWC' ) { //Restriccion para que los descuentos de clase ZPWC sean solo para clientes Droguistas es decir 100
          if ( $_POST[ 'Grp1' ] == '100' && ( int )$row[ "descuento_adg" ] > 0 ) {
            $descripcion = 'OFERTA EXCLUSIVA CLIENTE WEB : ' . utf8_encode( trim( $row[ "descripcion" ] ) );
            $grupoAr = utf8_encode( trim( $row[ "grupo_articulos" ] ) );
            $fechaV = $vmcto ;
          } else {
  ...
  case 'U_NOTAS':
    ...
    if ($resultado) echo json_encode(['ok' => true, 'msg' => "Success"]);
    else echo json_encode(['ok' => false, 'msg' => "Faild"]);
    mssql_close();
    break;
  case 'EMAIL':
    {
