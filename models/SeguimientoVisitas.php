...
switch ($_POST['op']) {
	case 'VENDEDOR': {
			...
			switch ($_SESSION["ses_RolesId"]) {
				case '14': {
						$sql .= " ID =" . $_SESSION["ses_Id"] . " AND";
					}
					break; //VENTAS EXTERNAS
				case '11': {
						$sql .= " ID =" . $_SESSION["ses_Id"] . " AND";
					}
					break; //TRANSF. INTERNAS
				case '11': {
						$sql .= " ID =" . $_SESSION["ses_Id"] . " AND";
					}
					break; //TRANSF. EXTERNAS
				case '12': {
						$sql .= " ID =" . $_SESSION["ses_Id"] . " AND";
					}
					break; //TELEVENTAS
				case '1': {
						$sql .= "  (ID_DPTO = " . $_POST['tipo'] . " OR ISNULL(ROLES_ID,0)=1   ) AND ";
					}
					break;
				default: {
						$sql .= "  ID_DPTO = " . $_POST['tipo'] . " AND ";
					}
					break;
			}
	...
	case 'Clientes_zona': {
			if ($_POST["id"] == 0) {
				$id_ejecutivo = $_SESSION["ses_Id"];
			} else {
				$id_ejecutivo = $_POST["id"];
			}
